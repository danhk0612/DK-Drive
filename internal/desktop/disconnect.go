...
// Windows session shutdown cannot wait for interactive confirmation. Try a
// normal detach first, then preserve local staging and force-detach any mount
// that still has open handles or a remembered close failure.
func disconnectProfilesForSessionEnd(manager *connection.Manager, profiles []config.SavedProfile) disconnectResult {
	return disconnectProfiles(manager, profiles, func(config.SavedProfile, error) bool { return true })
}

...
일반 FTP, Implicit FTPS와 WinFsp 마운트는 아직 실환경 검증 전이다.
...
이는 명시적 Flush 없이 쓰기 직후 조회하는 순서에 대한 수정이며, Windows/NAS
재검증 전에는 마운트 기본 파일 작업을 통과로 표시하지 않는다. 기존 진단 파일은
보존하고 새 파일로 `Set-Content` 직후 `Get-Content`를 재검증한다.

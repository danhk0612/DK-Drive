...
일반 FTP, Implicit FTPS, 쓰기와 WinFsp 마운트는 아직 실환경 검증 전이다.
...
아직 확정하지 않았다. 이동·이름 변경·삭제까지의 쓰기 검증은 미완료다.
...
읽기 내용 일치, 변경 명령의 중복 전송 방지를 확인한다. 실서버 재검증에서는
두 FTP 실행 파일을 다시 빌드하고 원래 시작 경로에서 `-write-test`를 실행한다.
기존 실패 폴더는 진단이 끝날 때까지 보존하며 성공으로 기록하지 않는다.
...
- Explicit FTPS 목록·읽기 실서버 검증 완료; 쓰기·재연결·Windows 마운트 검증 미완료

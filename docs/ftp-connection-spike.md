...
일반 FTP 모드의 읽기 전용·탐색기 UI·제어 연결 단절 복구는 아직 실환경
검증 전이다. 앞서 Explicit FTPS에서 통과한 결과와 혼동하지 않는다.
...
- Explicit FTPS 읽기 전용 마운트 기본 검증 완료; 기존 파일 덮어쓰기·이름 변경·삭제 차단 실환경 검증 전
- Explicit FTPS 읽기 전용 탐색기 목록·폴더 이동·텍스트 열기 검증 완료
- Explicit FTPS 탐색기·메모장 읽기/쓰기 기본 작업 검증 완료
- 일반 FTP의 읽기 전용·탐색기 UI·제어 연결 단절 복구 실환경 검증 전

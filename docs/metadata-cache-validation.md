...
### 다음 수동 확인 (성능 재측정·재빌드 불필요)

현재 빌드를 계속 사용한다. WebDAV와 Explicit FTPS에서 각각 새 임시 폴더를 만들어
아래 동작을 확인하고 사용한 테스트 파일만 정리한다.

1. 텍스트 파일 생성 → 저장 직후 재열기 → 더 긴 내용으로 덮어쓰기 → 즉시 내용·크기 확인.
2. 파일 이름 변경 → 하위 폴더로 이동 → 상위 폴더 이름 변경 → 파일과 테스트 폴더 삭제.
3. 테스트 파일을 다시 만든 뒤 읽기 전용으로 재연결한다. 읽기는 유지되고,
   기존 파일 덮어쓰기·이름 변경·파일 삭제·폴더 삭제가 각각 차단되는지 확인한다.
4. 탐색기와 편집기를 닫고 일반 연결 해제 → 재연결 → 정상 종료를 확인한다.

보고 양식: `프로토콜 / 즉시 내용·크기 / 이동·이름 변경·삭제 / 읽기 전용 차단 / 일반 해제`.
이미 수행한 항목은 재실행하지 않고 결과만 알려주면 된다.

그다음 로그인 후 한 번만 자동 실행·자동 연결, 정상 재시작, 업로드 실패 보존 및
복구를 기존 desktop-validation 절차에 따라 확인한다. 실제 실패 복구 검증은 기존
원격 사용자 파일을 사용하지 않으며, 확인되지 않은 프로토콜은 미검증으로 남긴다.

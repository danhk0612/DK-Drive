...
- [x] Windows 파일 작업
  ...
  - [x] 다른 폴더로 이동과 삭제
  - [x] 테스트 파일과 폴더 정리
- [x] Windows 탐색기와 메모장 실사용
  - [x] 탐색기에서 폴더와 텍스트 파일 생성
  - [x] 메모장에서 저장, 종료 후 다시 열어 내용 확인
  - [x] 탐색기에서 이름 변경과 루트 이동
  - [x] 탐색기에서 파일과 테스트 폴더 삭제
...
- [x] WebDAV 드라이브를 `X:`에 읽기 전용 모드로 마운트
- [x] 목록과 기존 파일 읽기 허용
- [x] 새 파일 생성 차단
- [x] 기존 파일을 Windows `ReadOnly` 속성으로 표시
- [x] 차단된 파일이 원격에 생성되지 않음

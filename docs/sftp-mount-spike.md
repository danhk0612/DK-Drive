...
서버 반영에 성공하거나 읽기가 끝난 스테이징 파일은 파일 핸들 정리가 완료되면
삭제한다. WinFsp와 Windows의 핸들 정리 시점에 따라 수 초 지연될 수 있다.
업로드가 실패한 파일은 데이터 보존을 위해 캐시 폴더에 남긴다.
...
- SFTP에서 별도 생성 시간과 접근 시간을 제공하지 않아 Windows의 생성 시간,
  접근 시간과 수정 시간에는 SFTP 수정 시간을 동일하게 표시한다.
- Windows 읽기 전용 파일 속성은 SFTP 파일의 쓰기 권한 비트로 변환한다.
  Hidden, System, Archive 등 대응값이 없는 속성 변경은 지원하지 않는다.
...
- 통과: 네트워크 어댑터 재시작 후 마운트 프로세스 재시작 없이 파일 읽기
- 통과: 재연결 전후 TCP 로컬 포트 변경으로 새 SSH/SFTP 세션 생성 확인
- 통과: 사용자 지정 캐시 폴더 생성
- 통과: 파일이 열린 동안 캐시 폴더에 스테이징 파일 유지
- 통과: 원격 반영 및 파일 핸들 정리 후 스테이징 파일 삭제
- 통과: 캐시를 거쳐 저장한 UTF-8 한글 파일 재읽기
- 통과: 파일 크기와 파일·디렉터리 기본 속성 조회
- 통과: Windows에서 지정한 수정 시간을 SFTP 서버에 반영
- 통과: 디렉터리 열거와 개별 파일 조회에서 동일한 원격 수정 시간 표시
- 통과: Windows 읽기 전용 속성 설정 후 쓰기 차단 및 속성 해제
- 통과: `Remove-Item -Force`로 파일과 폴더 삭제
- 확인: SSH `stat`과 SFTP 백엔드가 동일한 원격 수정 시간을 반환
